...
) const
{
    ...
    forAll(facesToRemove, i)
    {
        ...
        if(test){
            Info << "own: "<<own<<", nei: "<<nei<< endl;
            Info << "region0: "<<region0<<", region1: "<<region1<< endl;
        }
        if (region0 == -1)
        {
            if (region1 == -1)
            {
                ...
                if(test){
                    Info<< "new region added to regionMaster, nRegions = "<<nRegions<< ", regionMaster[nRegions] = "<<own<< endl;
                }
                nRegions++;
            }
            else
        ...
        else
        {
            ...
            else if (region0 != region1)
            {
                ...
                if(test){
                    Info<< "master0 = " << master0<<", master1 = "<<master1<< endl;
                    Info<< "regionMaster updated, freedRegion = "<<freedRegion<<", keptRegion = "<<keptRegion<< endl;
                    Info<< "regionMaster[keptRegion] = "<<regionMaster[keptRegion]<< endl;
                }
            }
        }
        if(test){
            Info<<"owner cell region: "<<cellRegion[own]<<", neigbour cell region: "<<cellRegion[nei]<< endl;
            Info << "Number of regions: "<<nRegions<< endl;
            Info << "region master: "<<nRegions<< endl;
            for(int i=0;i<nRegions;i++){
                Info<<regionMaster[i]<<endl;
            }
        }
    }
    ...
    if(test){
        Info<<"Region master: "<<regionMaster;
    }
    // Various checks
