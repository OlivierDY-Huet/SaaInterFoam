...
) const
{
	...
	/*
	https://develop.openfoam.com/Development/openfoam/-/issues/1746
	scalar deltaReceding = theta0_- thetaR_
	scalar deltaAdvancing = thetaA_- theta0_
	return theta0_ + deltaReceding*tanh(uwall/uTheta_)*(1-pos(uwall)) + deltaAdvancing*tanh(uwall/uTheta_)*pos(uwall)
	*/
}
